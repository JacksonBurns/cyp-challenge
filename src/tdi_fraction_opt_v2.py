"""Task 5: expected-MCC fraction machinery on the NEW blended TDI OOF.

3A4: rank-z 0.7*base + 0.3*emb (best MCC from tdi_emb_blend); 2D6: base only.
Reuses tdi_fraction_opt's functions. Writes cache/tdi_fraction_optima_v2.json.
"""
import json
import os
import sys
...
# blend on labeled rows (rank space), keeping base for 2D6
probs = {}
for iso in ISO:
    y = Y[f"{iso}_is_TDI"].values
    lab = ~pd.isna(y)
    pb, pe = base[iso][lab], emb[iso][lab]
    zb = (pb - pb.mean()) / pb.std()
    ze = (pe - pe.mean()) / pe.std()
    probs[iso] = 0.7 * zb + 0.3 * ze if iso == "CYP3A4" else zb
...
for iso in ISO:
    ...
    rates, _ = rank_conditional_curve(probs[iso], yl)
    ...
    out[iso] = {
        ...
        "weighted_mcc_at_f": {f"{f:.2f}": round(float(weighted[int(round((f - 0.05) / 0.01))]), 4)
                              for f in [0.08, 0.15, 0.25, 0.28, 0.33, 0.37, 0.43, 0.50]},
    }
    print(iso, "post-opt f:", out[iso]["posterior_optimal_frac"], "E[MCC]", out[iso]["posterior_optimal_mcc"],
          "minimax f:", out[iso]["minimax_optimal_frac"], flush=True)
    print("  ", out[iso]["weighted_mcc_at_f"], flush=True)
json.dump(out, open(os.path.join(CACHE, "tdi_fraction_optima_v2.json"), "w"), indent=2)
