"""TDI classifier with CheMeLeon embeddings concatenated (mirror of run_tdi.py).

Same scaffold folds (seed=7), same NN-positive features (self-excluded), same
classifier params; X = FP+RDKit (+optional CheMeLeon emb). Writes:
  cache/tdi_emb_test_probs.csv  (SMILES, CYP2D6_proba, CYP3A4_proba)
  cache/tdi_emb_oof.npz         (X_train-dedup row order, like run_tdi.py)
  cache/tdi_emb_cv.json
...
def main(use_emb=True):
    ...
    if use_emb:
        ...
        Xbase = np.hstack([Xbase, Etr])
        Xte_base = np.hstack([Xte_base, Ete])
    ...
    Btr = (pd.read_parquet(os.path.join(CACHE, "X_train.parquet")).drop_duplicates("SMILES").reset_index(drop=True)[FP_COLS].values > 0).astype(np.float32)
    ...
    np.savez(os.path.join(CACHE, f"tdi_{tag}_oof.npz"), **oof_store)
    with open(os.path.join(CACHE, f"tdi_{tag}_cv.json"), "w") as fh:
        json.dump(out_cv, fh, indent=2)


if __name__ == "__main__":
    main(use_emb=(len(sys.argv) < 2 or sys.argv[1] != "base"))
